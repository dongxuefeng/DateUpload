...
public:
	...
	c_time(const char * strdatetime)
	{
		set_time(strdatetime);
	};


	void set_time(const char * strdatetime) //yyyy-MM-dd HH:mm:ss:mms
	{
	...
	static long sec_between(c_time& ct1,c_time& ct2)
	{
		long itmp = 0;
		itmp = (ct1.mday - ct2.mday);
		itmp *= 24 * 3600;
		itmp += (ct1.msec - ct2.msec);
		return itmp;
	};
